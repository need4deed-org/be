...
// Restores the real historical match date for rows whose
// opportunity_volunteer.created_at (the de-facto "date matched" column — no
// dedicated column exists, see be#787) was overwritten with a migration-run
// timestamp instead of the actual date. Source: legacy Notion export
// "Volunteers.csv" (be#804), columns ID (volunteer nid), Matched (date),
// "VO matched ID " (comma-separated opportunity nids, one row's date applies
// to every opportunity id listed for that volunteer).
//
// Pairs as [volunteerNid, opportunityNid, matchedDate]. matchedDate is the
// ISO date parsed from the CSV's actual month-name format (e.g.
// "April 1, 2026") — English, not German as originally assumed when the
// issue was filed; there is no ambiguity in month names between the two
// locales here since none of the source rows used non-English month names.
const MATCHED_DATES: [string, string, string][] = [
  ...
  ["VOLVO-377", "VOL-847", "2026-03-18"],
];

export class RestoreMatchDatesFromNotionCsv1785432386411
  implements MigrationInterface
{
  name = "RestoreMatchDatesFromNotionCsv1785432386411";

  // Safe to re-run: only ever updates created_at, and only when it differs
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (unresolved.length > 0) {
      console.warn(
        `RestoreMatchDatesFromNotionCsv: ${unresolved.length} pair(s) did not resolve to an existing matched relation:`,
        unresolved,
      );
    }
    ...
    await queryRunner.query(`
      ...
      SET created_at = p.matched_date::timestamp, updated_at = now()
      FROM (VALUES ${values}) AS p(vol_nid, opp_nid, matched_date)
      ...
      WHERE opportunity_volunteer.volunteer_id = v.id
        ...
        AND opportunity_volunteer.created_at IS DISTINCT FROM p.matched_date::timestamp;
    `);
  }
